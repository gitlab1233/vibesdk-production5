...
// ✅ caminho corrigido para urls.ts
...
export class CodingAgentController extends BaseController {
  ...
  static async startCodeGeneration(
    ...
    try {
      ...
      const { sandboxSessionId, templateDetails, selection } =
        await getTemplateForQuery(env, inferenceContext, query, this.logger);

      const websocketUrl = `${
        url.protocol === 'https:' ? 'wss:' : 'ws:'
      }//${url.host}/api/agent/${agentId}/ws`;
  ...
  static async connectToExistingAgent(
    ...
    try {
      ...
      try {
        ...
        return CodingAgentController.createErrorResponse<AgentConnectionData>(
          `Agent instance not found or unavailable: ${
            error instanceof Error ? error.message : String(error)
          }`,
          404
        );
  ...
  static async deployPreview(
    ...
    try {
      ...
      const appError =
        CodingAgentController.handleError(
          error,
          'deploy preview'
